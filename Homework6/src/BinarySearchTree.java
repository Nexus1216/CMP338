	...
	protected T retrieveItem(TreeNode<T> tNode , KT searchKey) {
		...
		protected TreeNode<T> deleteItem(TreeNode<T> tNode, KT searchKey) {
			...
			if (tNode == null ) { 
				...
				T nodeItem = tNode. item; 
				if (searchKey.compareTo(nodeItem.getKey()) < 0) { // item is in the root of some subtree 
					tNode = deleteNode(tNode); // delete the item
				}// else search for the item 
				else if (searchKey. compareTo(nodeItem. getKey( ) ) < 0) { 
					// search the left subtree newSubtree 
			...
			public boolean isBalanced(TreeNode<T> tNode){  
				   
				if (balanceHeight(tNode) == -1) { 
			         return false;  
			    }  
			    return true;  
			}  
			
			public void balanceTree(T data[], int first, int last) {
		        if (first <= last) {
		    ...
		    public void balanceTree(T data[]) {
		    	balanceTree(data,0,data.length-1);
		    }
			   
			   
			  
			  
			public int balanceHeight(TreeNode<T> tNode){
				if (tNode == null){  
			         return 0;  
			    }  
			  
			    int left = balanceHeight(tNode.leftChild);  
			    int right = balanceHeight(tNode.rightChild);  
			...
			public int treeHeight(TreeNode<T> tNode){
				if (tNode == null){
				        return 0;
				}
				else{
					return 1+
				    Math.max(treeHeight(tNode.leftChild), treeHeight(tNode.rightChild));
				}
			}
